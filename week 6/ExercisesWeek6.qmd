...
# Basic counts by time category
print("Count of sequences by days category:")
print(table(day_categories))

# Look for exceptions to temporal patterns
# Calculate mean distance from root for each time category
node_depths <- node.depth.edgelength(rooted_tree)
tip_depths <- node_depths[1:length(rooted_tree$tip.label)]
names(tip_depths) <- rooted_tree$tip.label

# Group depths by time category
depths_by_category <- list()
for (cat in unique(day_categories)) {
  cat_indices <- which(day_categories == cat)
  depths_by_category[[cat]] <- tip_depths[cat_indices]
}

# Print mean depth by category
print("Mean evolutionary distance from root by category:")
for (cat in names(depths_by_category)) {
  print(paste(cat, ":", mean(depths_by_category[[cat]])))
}

# Find outliers - early sequences that are deeply nested or late sequences near the root
early_threshold <- quantile(depths_by_category[["Early"]], 0.75)
late_threshold <- quantile(depths_by_category[["Late"]], 0.25)

print("Potential outliers - Early sequences deeper than expected:")
deep_early <- which(day_categories == "Early" & tip_depths > early_threshold)
if (length(deep_early) > 0) {
  print(rooted_tree$tip.label[deep_early])
}

print("Potential outliers - Late sequences shallower than expected:")
shallow_late <- which(day_categories == "Late" & tip_depths < late_threshold)
if (length(shallow_late) > 0) {
  print(rooted_tree$tip.label[shallow_late])
}

# Check monophyly of time points
print("Are time categories monophyletic?")
for (cat in c("Early", "Intermediate", "Late")) {
  tips <- which(day_categories == cat)
  is_mono <- is.monophyletic(rooted_tree, tips)
  print(paste(cat, "is monophyletic:", is_mono))
}

# Print some information to help with question 6
if (exists("sequences2")) {
  # For question 6
  print("Information for second patient (sequences2):")
  seq0_indices <- which(annotations2$DaysFromSeroconversion == 0)
  if (length(seq0_indices) > 0) {
    print(paste("Number of sequences at seroconversion (0 days):", length(seq0_indices)))
  }
}
```
...
# Load the sequences for the second patient
sequences2 <- read.dna("sequences2.fasta", format = "fasta")

# Load annotations for the second patient
annotations2 <- read.csv("annot2.csv")

# Calculate pairwise distances using Jukes-Cantor model
dist_matrix2 <- dist.dna(sequences2, model = "JC69")

# Infer neighbor joining tree
nj_tree2 <- bionj(dist_matrix2)

# Root the tree using the reference sequence
rooted_tree2 <- root(nj_tree2, outgroup = 1, resolve.root = TRUE)

# Ladderize the tree for better visualization
rooted_tree2 <- ladderize(rooted_tree2)

# Manually expand the horizontal component of the edges
stretched_tree2 <- rooted_tree2

# Find the reference branch index
ref_edge_idx2 <- which(stretched_tree2$edge[,2] == 1)

# Use a moderate stretching factor
for (i in 1:nrow(stretched_tree2$edge)) {
  if (i != ref_edge_idx2) {
    stretched_tree2$edge.length[i] <- stretched_tree2$edge.length[i] * 6
  } else {
    # Slightly shorten the reference branch
    stretched_tree2$edge.length[i] <- stretched_tree2$edge.length[i] * 0.8
  }
}

# Extract the part of the tip labels after the last period
clean_tip_labels2 <- sapply(stretched_tree2$tip.label, function(label) {
  parts <- strsplit(label, "\\.")[[1]]
  return(parts[length(parts)])
})

# Match cleaned tip labels with annotation names
day_lookup2 <- annotations2$DaysFromSeroconversion
names(day_lookup2) <- annotations2$Name

# Get days for each tip
tip_days2 <- numeric(length(clean_tip_labels2))
for (i in 1:length(clean_tip_labels2)) {
  matches <- grep(clean_tip_labels2[i], annotations2$Name, fixed = TRUE)
  
  if (length(matches) > 0) {
    tip_days2[i] <- annotations2$DaysFromSeroconversion[matches[1]]
  } else {
    tip_days2[i] <- NA  # Set NA for reference sequence
  }
}

# Define categories for coloring based on actual days in dataset
day_categories2 <- rep("Reference", length(tip_days2))
day_categories2[!is.na(tip_days2) & tip_days2 == 0] <- "Seroconversion"
day_categories2[!is.na(tip_days2) & tip_days2 > 0 & tip_days2 <= 50] <- "Early"
day_categories2[!is.na(tip_days2) & tip_days2 > 50 & tip_days2 <= 120] <- "Intermediate"
day_categories2[!is.na(tip_days2) & tip_days2 > 120] <- "Late"

# Set colors based on categories
color_mapping2 <- c(
  "Reference" = "darkblue",
  "Seroconversion" = "purple",
  "Early" = "green3", 
  "Intermediate" = "orange",
  "Late" = "red"
)

tip_colors2 <- color_mapping2[day_categories2]

# Set up plotting area with more space on the right
par(mar = c(1, 1, 3, 8), xpd = TRUE)

# Plot the stretched tree
plot(stretched_tree2, show.tip.label = FALSE, 
     main = "HIV Phylogenetic Tree for Second Patient\nColored by Days from Seroconversion",
     cex = 0.8,
     edge.width = 1.2,
     direction = "rightwards")

# Add colored dots for each tip
tiplabels(pch = 19, col = tip_colors2, cex = 1.2)

# Create a legend positioned on the right side
plot_dims <- par("usr")
legend(x = plot_dims[2] * 0.8,
       y = plot_dims[4] * 0.95,
       legend = c("Reference", 
                  "Seroconversion (0 days)",
                  "Early (31 days)", 
                  "Intermediate (59 days)", 
                  "Late (115-199 days)"),
       pch = 19, 
       col = color_mapping2,
       cex = 1.2, 
...
print(paste("Number of sequences >100 days after seroconversion:", 
           sum(!is.na(tip_days2) & tip_days2 > 100)))

# Calculate mean distance from root for each time category
node_depths2 <- node.depth.edgelength(rooted_tree2)
tip_depths2 <- node_depths2[1:length(rooted_tree2$tip.label)]
names(tip_depths2) <- rooted_tree2$tip.label

# Print number of sequences at each time point
print("Number of sequences by time point:")
print(table(day_categories2))
```
