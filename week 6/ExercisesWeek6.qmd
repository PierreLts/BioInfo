...
1.  ***With weak selection (***$s=0.01$*):*
    -   *The mutant fraction increases from 0.01 to 0.01050732, which is precisely a 5.0732% increase*
    -   *The fold increase is exactly 1.050732*
...
2.  ***With strong selection (***$s=0.5$*):*
    -   *The mutant fraction increases from 0.01 to 0.1095721, which is a 10.95721-fold increase*
    -   *In percentage terms, this represents a dramatic 995.721% increase*
...
3.  ***Implications for experimental evolution**:*
    ...
    -   *For mutations with small fitness advantages (like* $s=0.01$*), genetic drift during bottlenecks might counteract selection, requiring many more passages for fixation*
    -   *Mutations with large fitness advantages (like* $s=0.5$*) would be expected to dominate even after a few cycles*
4.  ***Mathematical explanation**:*
    -   *The exponential term* $e^{st}$ *amplifies even small differences in* $s$ *over time*
    -   *For* $s=0.01$ *and* $t=5$*, we get* $e^{0.01 \times 5} = e^{0.05} \approx 1.0513$
    -   *For* $s=0.5$ *and* $t=5$*, we get* $e^{0.5 \times 5} = e^{2.5} \approx 12.1825$
    -   *This demonstrates why even small fitness advantages can eventually lead to fixation given enough time, while large fitness advantages lead to rapid change*
...
*The variance (658.1651) is dramatically larger than the mean (5.584), with a variance-to-mean ratio of approximately 117.87. This represents extreme overdispersion, as a Poisson distribution would have a variance-to-mean ratio of 1*

*This extreme overdispersion is caused by two key factors:*

***The timing of mutations:** Mutations that occur early during the growth phase generate many more descendants than late mutations due to the exponential growth process*

***The selection advantage (s=0.01):** Mutant lineages grow faster than wild-type cells, which amplifies the effect of early mutations*

*These factors create a highly skewed distribution with many samples having few mutants, while a small number of samples have very large numbers of mutants. This results in the observed high variance without substantially affecting the mean*
...
*The results show extreme variability in mutant counts across the 10 replicates:*

*4 replicates produced zero mutants 5 replicates had low mutant counts (1-7) 1 replicate (replicate 2) had 70 mutants, which is dramatically higher than all others*

*This pattern creates striking statistical properties:*

*Mean mutant count: 9 Standard deviation: 21.61 Coefficient of variation: 2.40 (SD/mean)*

*The standard deviation is more than twice the mean value, indicating extreme overdispersion. This directly connects to our findings in Question 3, where we observed a variance-to-mean ratio of approximately 118 for mutants arising from single cells. This variability can be explained by two key stochastic factors:*
...
*The outlier in replicate 2 (70 mutants) likely represents a case where either:*

*Multiple mutation events occurred by chance, or A mutation occurred very early in the growth phase, allowing the mutant lineage to exploit its selective advantage (s=0.01) over more generations*

*This extreme variability has important implications for experimental evolution:*

*Replicate populations can follow dramatically different evolutionary trajectories Beneficial mutations may establish in some replicates but not others, purely by chance Population bottlenecks amplify this variability by randomly sampling from these highly variable distributions*

*The results illustrate why large numbers of replicates are crucial in experimental evolution studies to account for the inherent stochasticity in mutation occurrence and establishment.*
...
### *a) Probability distribution for the number of mutants sampled*

*The number of mutants* $k$ *sampled to form the next bottleneck follows a **binomial distribution**.*

*This is because: - We're sampling* $K$ *individuals from a large population - Each individual sampled is either a mutant (with probability* $x'$*) or wild-type (with probability* $1-x'$*) - Each sampling event is independent - We're interested in the number of "successes" (mutants) in* $K$ *trials*

### *b) Formula for the probability* $P(k)$

*The probability of sampling exactly* $k$ *mutants when forming a bottleneck of size* $K$ *is given by:*

$$P(k) = \binom{K}{k} (x')^k (1-x')^{K-k}$$

*Where: -* $k$ *is the number of mutants sampled (*$0 \leq k \leq K$*) -* $K$ *is the bottleneck size -* $x'$ *is the mutant fraction after growth -* $\binom{K}{k} = \frac{K!}{k!(K-k)!}$ *is the binomial coefficient*

*This formula represents the probability mass function of the binomial distribution* $B(K, x')$*.*
...
*The number of mutants sampled for the next bottleneck follows a binomial distribution with parameters* $K = 5 \times 10^6$ and probability $x' = 0.01050732$. The mean is 52,535 mutants with a standard deviation of 228.73, and the distribution approximates a normal curve as shown in the histogram.
...
plot(0:n_bottlenecks, x_values[, 1], type = "l", col = colors[1],
     ...
     ylim = c(0, 1), main = "Serial passage model with selection (s=0.01)",
     lwd = 2)
...
*Analysis of Serial Passage Results*

*The simulation results show remarkably consistent trajectories with all 10 realizations reaching near-complete fixation (\>0.999) by the end of 300 bottlenecks. This reveals:*

*Deterministic dynamics dominate: The trajectories follow an almost perfect sigmoidal curve with minimal random variation. Selection overwhelmingly outweighs drift: With Ns = 50,000 (K×s = 5×10\^6 × 0.01), selection is approximately 50,000 times stronger than genetic drift, explaining why stochastic effects are nearly invisible. Minimal sampling variance: For a binomial process with such large populations, the standard deviation is negligibly small relative to the population size (\~0.022% at maximum variance).*

*Implications This simulation demonstrates a practically deterministic regime of evolution where:*

*Predictable outcomes: Large population sizes with moderate selection coefficients produce highly predictable evolutionary trajectories. Limited role for chance: Unlike in smaller populations where drift can lead to loss of beneficial mutations, here selection reliably drives beneficial mutations to fixation.*

*To observe more stochastic effects and trajectory diversity, we would need a much smaller bottleneck size (e.g., K = 1,000) or a weaker selection coefficient (e.g., s = 0.0001).*
