...
# Infer neighbor joining (NJ) tree
nj_tree <- bionj(dist_matrix)

# Root the tree using the first sequence (reference) as outgroup
...
# Extract days from seroconversion from annotations
days <- annotations$DaysFromSeroconversion
names(days) <- annotations$Name

# Create a color palette for the gradient based on days
# First, get the days for each tip in the tree
tip_days <- days[rooted_tree$tip.label]

# Replace NA with a distinctive value (for the reference sequence)
tip_days[is.na(tip_days)] <- -100  # Using -100 to distinguish reference sequence

# Create color palette (blue to red gradient)
col_palette <- colorRampPalette(c("blue", "green", "yellow", "red"))
tip_colors <- num2col(tip_days, col.pal = col_palette)

# Plot the tree
plot(rooted_tree, cex = 0.8, type = "phylogram", 
     main = "HIV Phylogenetic Tree Colored by Days from Seroconversion")

# Add tip labels showing days from seroconversion
tiplabels(text = tip_days, bg = tip_colors, frame = "circle", cex = 0.8)

# Add a legend
legend("topright", 
       legend = c("Reference", "Early", "Intermediate", "Late"),
       pch = 21, 
       pt.bg = col_palette(4), 
       pt.cex = 2, 
