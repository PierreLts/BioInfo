...
# Calculate x' (mutant fraction after growth)
x_prime <- (x * exp(s * t)) / ((1 - x) + x * exp(s * t))

# Sample k from binomial distribution
...
*The number of mutants sampled for the next bottleneck follows a binomial distribution with parameters $K = 5 \times 10^6$ and probability $x' = 0.01050732$. The mean is 52,535.05 mutants with a standard deviation of 228.73, and the distribution approximates a normal curve as shown in the histogram.*
