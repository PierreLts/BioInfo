...
# First, let's check the structure of our sequences
class(sequences2)
typeof(sequences2)
dim(sequences2)

# Look at sequence names
seq_names <- attr(sequences2, "dimnames")[[1]]
cat("First 10 sequence names:", head(seq_names, 10), "\n")

# Find day 0 sequences
day0_pattern <- "\\.\\-\\.0\\.\\-\\."  # Pattern for ".-.0.-."
day0_indices <- grep(day0_pattern, seq_names)
cat("Day 0 sequences found:", length(day0_indices), "\n")

# Create a new, clean DNAbin matrix for day 0 sequences
if(length(day0_indices) >= 2) {
  # Get the dimensions and structure right
  if(is.matrix(sequences2)) {
    # If sequences2 is a matrix (aligned sequences)
    day0_sequences <- sequences2[day0_indices, ]
  } else {
    # If sequences2 is a list (potentially unaligned)
    day0_sequences <- as.matrix(as.character(sequences2[day0_indices]))
  }
  
  # Verify structure
  cat("Structure of day0_sequences:", dim(day0_sequences), "\n")
...
**Comparison with Flu Virus:**

HIV at Seroconversion (single patient
