...
# Set up plotting area with more space on the right
par(mar = c(1, 20, 1, 20), xpd = TRUE)  # Increased right margin
...
[**Calculation for specific parameter values**]{style="color:#0066FF;"}
...
[**Comparison and commentary**]{style="color:#0066FF;"}
...
[- For $s=0.01$ and $t=5$, we get $e^{0.01 \times 5} = e^{0.05} \approx 1.0513$]{style="color:#0066FF;"} [- For $s=0.5$ and $t=5$, we get $e^{0.5 \times 5} = e^{2.5} \approx 12.1825$]{style="color:#0066FF;"}
...
# Mean line
abline(v = mean_k, col = "#DE0099", lwd = 2)
```
